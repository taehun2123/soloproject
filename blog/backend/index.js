...
app.post("/checkedid", (req,res) => {
  ...
  fs.readFile(signupPath, 'utf8', (err, data) => {
    if(err){
      ...
      try {
        const readId = JSON.parse(data);

        const correctId = readId.find((item) => item.id===loadId);
        if(correctId){
          res.json({ success: false, message: "중복된 아이디입니다."});
        } else {
...
app.post("/signup", (req,res)=>{
  ...
  fs.writeFile(signupPath, jsonData, 'utf8', (err) => {
  if (err) {
  console.error('파일 저장 중 에러 발생:', err);
  } else {
  console.log('파일이 성공적으로 저장되었습니다.');
  ...
  fs.readFile(signupPath, 'utf8', (err, data) => {
    if(err){
      ...
      try {
        ...
        const user = readId.find((item) => item.id===loadLogin.id && item.pw===loadLogin.pw);
        if(user){
          req.session.user = user;
          res.json({ success: true, message: "로그인 되었습니다."});
        } else {
          res.json({ success: false, message: "아이디와 비밀번호를 확인해주세요."})
        }
      } catch (parseError) {
        console.error('JSON 파싱 중 에러 발생:', parseError);
        res.status(400).json({ message: '잘못된 JSON 데이터' }); 
